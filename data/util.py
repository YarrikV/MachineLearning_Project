...
def plot_learning_curve(learning_curve_result):
    ...
    save_path = os.path.join("visualization", "learning_curve.png")
    fig.savefig(save_path)

def plot_validation_curve(validation_curve_result, param_name, param_range, xscale="linear"):
    """Plots a validation curve. 
    Training and cross validation scores in function of a hyperparameter.
    
    :param learning_curve_result: The result of a call to validation_curve(...)
    :param param_name: name of the parameter for the validation curve
    :param param_range: range of the parameter for the validation curve
    :param xscale: scale on the x-axis: "linear" (default), "log", ...
    """
    train_scores, test_scores = validation_curve_result
    ...
    ax.set_title('Validation Curve', fontsize=16, fontweight='bold')
    save_path = os.path.join("visualization", "validation_curve.png")
    fig.savefig(save_path)

def plot_grid_search(cv_results, param_grid):
    # check if gridsearch is two-dimensional and contains C
    if len(param_grid) == 2 and 'classify_lr__C' in param_grid.keys():
        # assign names and ranges to hyperparameters
        for param_name, param_range in param_grid.items():
            if param_name == 'classify_lr__C':
                param_name_C = param_name
                param_range_C = param_range
            else:
                param_name_2 = param_name
                param_range_2 = param_range

        # plot cross validation curve
        # https://stackoverflow.com/questions/37161563/how-to-graph-grid-scores-from-gridsearchcv    
        # Get Test Scores Mean and std for each grid search
        scores_mean = cv_results['mean_test_score']
        scores_mean = np.array(scores_mean).reshape(len(param_range_C),len(param_range_2))

        scores_sd = cv_results['std_test_score']
        scores_sd = np.array(scores_sd).reshape(len(param_range_C),len(param_range_2))

        # Plot Grid search scores
        fig, ax = plt.subplots()

        # Param1 is the X-axis, Param 2 is represented as a different curve (color line)
        for idx, val in enumerate(param_range_C):
            ax.plot(param_range_2, scores_mean[idx,:], '-o', label='C=' + str(val))
        
        ax.set_title("Validation Curves", fontsize=16, fontweight='bold')
        ax.set_xlabel(param_name_2, fontsize=14)
        ax.set_ylabel('Cross Validation Scores', fontsize=14)
        ax.legend(loc="best", fontsize=14)
        ax.grid('on')

        save_path = os.path.join("visualization", "validation_curve_") + param_name_2 + ".png"
        fig.savefig(save_path)
    else:
        # error message
        print('This function only works for a parameter grid containing "classify_lr__C" and one other hyperparameter')

def plot_confusion_matrix(confusion_matrix_result, fold_number):
    ...
    fig = plt.figure()
    ax = fig.gca()
    ax.set_title('Confusion Matrix Fold {}'.format(fold_number), fontsize=16, fontweight='bold')
    ax.axis('off')
    disp = ConfusionMatrixDisplay(confusion_matrix=confusion_matrix_result)
    disp.plot(include_values=False, ax=ax)
    save_path = os.path.join("visualization", "confusion_matrix_fold_{}.png".format(fold_number))
    fig.savefig(save_path)

def show_misclassifications(estimator, X_train, y_train, cv_indices, fold_number, N):
    """Plots normalized confusion matrix and some example images of misclassifications
    for a certain fold

    :param estimator: estimator object; pipeline
    :param X_train: training data
    :param y_train: training labels
    :param cv_indices: indices corresponding to cross-validation strategy
    :param fold_number: fold number n=1,...,folds
    :param N: number of misclassified images that must be displayed (randomly chosen)
    """
    ...
    plot_confusion_matrix(conf_matr, fold_number)
    # show misclassified images for this fold
    miscl_indices_fold = np.nonzero(y_true!=y_pred)[0] # indices of misclassified images inside fold
    print('number of misclassified images:', len(miscl_indices_fold))
    # choose n random misclassified images
    miscl_indices_fold_rand = sorted(np.random.choice(miscl_indices_fold, N, replace=False))
    print('number of shown misclassified images:', len(miscl_indices_fold_rand))
    fig, ax = plt.subplots(len(miscl_indices_fold_rand), 2, figsize=(3,len(miscl_indices_fold_rand)))
    # plot n misclassified images
    for i, miscl_idx_fold in enumerate(miscl_indices_fold_rand):
        miscl_idx = test_indices[miscl_idx_fold] # indices of misclassified images in complete X_train
        true_class = y_true[miscl_idx_fold] # true class label
        pred_class = y_pred[miscl_idx_fold] # pred class label       
        image_true = X_train[miscl_idx] # misclassified image
        image_pred = X_train[np.where(y_train == pred_class)[0][0]] # choose example image of pred class
        ax[i,0].set_title(CLASSES[true_class], fontsize=14)
        ax[i,1].set_title(CLASSES[pred_class], fontsize=14)
        ax[i,0].imshow(image_true)
        ax[i,1].imshow(image_pred)
        ax[i,0].set_axis_off()
        ax[i,1].set_axis_off()
    fig.suptitle('Misclassifications Fold {}'.format(fold_number), fontsize=16, fontweight='bold', y=1.0001)
    fig.tight_layout()
    save_path = os.path.join("visualization", "misclassif_fold_{}.png".format(fold_number))
    fig.savefig(save_path)
...
def plot_correlation_matrix(df, method='pearson', number_of_ticks=50):
    ...
    cax = ax.matshow(df.corr(method=method), vmin=-1, vmax=1)
